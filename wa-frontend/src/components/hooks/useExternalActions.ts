...
export const useExternalActions = () => {
    ...
    const handleSetBadge = (count: number) => {
    ...
    // Fungsi Copy Gambar
    const handleCopy = async (contextMenuImage: { x: number, y: number, chatMsg: Bubble | null }) => {
        try {
    ...
    const handleDownload = async (contextMenuImage: { x: number, y: number, chatMsg: Bubble | null }) => {
        try {
            // Bridge to WinForms for desktop notification
            if (!contextMenuImage.chatMsg?.content?.body?.url) return;
            if ((window as any).chrome?.webview) {
    ...
    const handleShowNotif = async (title: string, msg: string) => {
        if ((window as any).chrome?.webview) {
            (window as any).chrome.webview.postMessage({
                type: 'SHOW_NOTIFICATION',
                title: title,
                message: msg || 'Pesan baru'
            });
        }
    }
    ...
    const handleRefreshModule = async (module: string) => {
        try {
            if ((window as any).chrome?.webview) {
                ...
                console.log("Tidak berada di WinForms. gagal membuka module templates");
            }
        } catch {
            console.log("Tidak berada di WinForms. gagal membuka module templates");
        }
    ...
    const handleOpenModule = async (module: string) => {
        try {
            if ((window as any).chrome?.webview) {
                (window as any).chrome.webview.postMessage({
                    type: 'OPEN_MODULE',
    ...
    const handleCloseModule = async (module: string) => {
        try {
            if ((window as any).chrome?.webview) {
                (window as any).chrome.webview.postMessage({
                    type: 'CLOSE_MODULE',
    ...
    return { handleCopy, handleDownload, handleResendMessage, handleOpenModule, handleSetBadge, handleCloseModule, handleShowNotif,handleRefreshModule };
}
